...
class ImgDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(img).unsqueeze(0)/255.0, torch.tensor(int(self.ys[idx]))
...
class MnistModel(nn.Module):
    ...
    def forward(self, x):
        x = self.lin1(F.relu(F.avg_pool2d(self.bn1(self.conv1(x)), 3, 2)).flatten(1))
        return x
...
print(list_vars['conv1.weight'][0])
