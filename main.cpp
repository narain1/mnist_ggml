...
bool mnist_model_load(const std::string & fname, mnist_model & model) {
...

  {
...

    {
      ...
      ctx_size +=  n_input * n_hidden * ggml_type_sizef(GGML_TYPE_FP32);
      ctx_size += n_hidden * ggml_type_sizef(GGML_TYPE_FP32);

      ctx_size += n_hidden * n_classes * ggml_type_sizef(GGML_TYPE_FP32);
      ctx_size += n_classes * ggml_type_sizef(GGML_TYPE_FP32);

      printf("%s: ggml ctx size = %6.2f\n", __func__, ctx_size/(1024.0 * 1024.0));
    }

    // creating ggml content
    {
...

    {
      ...
      fin.read(reinterpret_cast<char *>(&&n_dims), sizeof(n_dims));
      {
        ...
        model.fc1_weight = ggml_new_tensor_2d(ctx, GGML_TYPE_FP32, model.hparams.n_input, model.hparams.n_hidden);
...

      {
        ...
        model.fc1_bias = ggml_new_tensor_1d(ctx, GGML_TYPE_FP32, model.hparams.n_hidden);
    ...
    // Fc2 layers
    {
      ...
      fin.read(reinterpret_cast<char *>(&n_dims), sizeof(n_dims));
      {
        ...
        model.fc2_weight = ggml_new_tensor_2d(ctx, GGML_TYPE_FP32, model.hparams.n_hidden, model.hparams.n_classes);
...
int mnist_eval(
  ...
  struct ggml_tensor * input = ggml_new_tensor_1d(ctx0, GGML_TYPE_FP32, hparams.n_input);
  memcpy(input-> data, digit.data(), ggml_nbytes(input));
  ggml_set_name(input, "input");

  // fc1 MLP = ax + b
  ggml_tensor * fc1 = ggml_add(ctx0, ggml_mul_mat(ctx0, model.fc1_weight, input), model.fc1_bias);
  ggml_tensor * fc2 = ggml_add(ctx0, ggml_mul_mat(ctx0, model.fc2_weight, ggml_relu(ctx0, fc1)), model.fc2.bias);

  ggml_tensor * probs = ggml_softmax(ctx0, fc2);
...
int wasm_random_digit(char * digitPtr) {
  auto fin = std::ifstream("models/mnist/ubyte", std::ios::binary);
  if (!fin) {
